...
def mill_monthly_summary():
    try:
        ...
        if not os.path.exists(CALENDAR_FILE):
            ...
            return render_template('mill_monthly_summary.html', title="Mill Monthly Summary", current_month=datetime.now().month)
        ...
        if row.empty:
            flash(f"No calendar entry found for month {month}.", "warning")
            return render_template('mill_monthly_summary.html', title="Mill Monthly Summary", current_month=datetime.now().month)
